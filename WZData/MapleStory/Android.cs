...
namespace WZData.MapleStory
{
    public class Android
    {
        ...
        public static Android Parse(WZProperty data, int id)
        {
            ...
            result.ActionMessages = data.Resolve("action").Children.ToDictionary(c => c.Key, c => c.Value.Children.Select(b => AndroidMessage.Parse(b.Value)).ToArray());

            result.DefaultEquips = data.Resolve("basic").Children.Select(c => ((WZPropertyVal<int>)c.Value).Value).Where(c => c != 0).ToArray();

            if (data.Children.ContainsKey("costume"))
            {
                WZProperty costume = data.Resolve("costume");
                result.PossibleFaces = costume.Resolve("face").Children.Select(c => ((WZPropertyVal<int>)c.Value).Value).Where(c => c != 0).ToArray();
                result.PossibleHairs = costume.Resolve("hair").Children.Select(c => ((WZPropertyVal<int>)c.Value).Value).Where(c => c != 0).ToArray();
                result.PossibleSkins = costume.Resolve("skin").Children.Select(c => ((WZPropertyVal<int>)c.Value).Value).Where(c => c != 0).ToArray();
            }

            if (data.Children.ContainsKey("info"))
            {
