...
namespace PKG1 {
    public class WZReader : BinaryReader {
        ...
        {
            if (readByte && this.ReadByte() == 0) return new byte[] { 0 };
        ...
        {
            if (readByte && this.ReadByte() == 0) return new byte[0];
            byte[] encryptionKey = encrypted == EncryptionType.None ? null : (encrypted == EncryptionType.GMS ? GMSKey : KMSKey);
            int length = this.ReadSByte();
            bool isUnicode = length > 0;
            if (isUnicode)
            {
        ...
        public bool ReadWZStringExpecting(out EncryptionType encryptedWith, string expecting, bool readByte = false)
        {
            ...
            EncryptionType[] types = new EncryptionType[] { EncryptionType.None, EncryptionType.GMS, EncryptionType.KMS };
            foreach (EncryptionType encrypted in types)
            {
                byte asciiMask = 0xAA;
                ushort unicodeMask = 0xAAAA;
        ...
        public string ReadWZString(bool readByte = false, EncryptionType encrypted = EncryptionType.None/*, int retryCount = 0*/) {
            if (readByte && this.ReadByte() == 0) return "";
            byte[] encryptionKey = (encrypted == EncryptionType.None || encrypted == (EncryptionType.KMS | EncryptionType.GMS)) ? null : (encrypted == EncryptionType.GMS ? GMSKey : KMSKey);
            ...
            if (!isUnicode){
                ...
                string res = Encoding.ASCII.GetString(asciiBytes);
        ...
        public byte[] GetDeDupedStringBytes(bool readByte = false, EncryptionType encrypted = EncryptionType.None) => GetWZStringAtOffsetBytes(ReadInt32(), readByte, encrypted);
        public string ReadDeDupedString(bool readByte = false, EncryptionType encrypted = EncryptionType.None) => ReadWZStringAtOffset(ReadInt32(), readByte, encrypted);
        ...
        public byte[] GetWZStringAtOffsetBytes(long offset, bool readByte = false, EncryptionType encrypted = EncryptionType.None)
        {
            ...
            byte[] result = GetWZStringBytes(readByte, encrypted);
        ...
        {
            byte t = ReadByte();
            return t == 0x80 ? new byte[] { t, ReadByte(), ReadByte(), ReadByte(), ReadByte() } : new byte[] { t };
        }
        ...
        {
            byte t = ReadByte();
            return t == 0x80 ? new byte[] { t }.Concat(ReadBytes(4)).ToArray() : new byte[] { t };
        }
        ...
        {
            byte t = ReadByte();
            return t == 0x80 ? new byte[] { t }.Concat(ReadBytes(8)).ToArray() : new byte[] { t };
        }
        ...
        internal IEnumerable<IEnumerable<byte>> DecryptPNGNested(Stream inData, uint length, byte[] wzKey = null)
        {
            ...
            using (BinaryReader reader = new BinaryReader(inData, Encoding.ASCII, true)) {
        ...
        byte[] Inflate(Stream data, uint dataLength)
        {
            ...
            using (DeflateStream deflator = new DeflateStream(data, CompressionMode.Decompress, true))
        ...
        internal byte[] GetPNGBytes(int width, int height, int format, bool isEncrypted, uint imageLength, byte[] encryptionKey = null)
        {
            ...
            catch (Exception ex)
            {
                if (isEncrypted && encryptionKey == null)
                {
                    ...
                    return GetPNGBytes(width, height, format, isEncrypted, imageLength, KMSKey);
                }
            }
        ...
        internal Image<Rgba32> ParsePNG(int width, int height, int format, bool isEncrypted, uint imageLength, byte[] encryptionKey = null)
        {
            ...
            try {
                ...
                {
                    BaseStream.Position = position;
                    return ParsePNG(width, height, format, isEncrypted, imageLength, KMSKey); // Should I try supporting GMSKey here? Not sure I've ever seen it needed.
                }
                else
            ...
            switch (format) {
                ...
                case 2:
                    ...
                    return Image.LoadPixelData<Bgra32>(sourceData, width, height).CloneAs<Rgba32>();
                case 513:
                    ...
                    return Image.LoadPixelData<Bgr565>(sourceData, width, height).CloneAs<Rgba32>();
                case 517:
                    width >>= 4;
                    height >>= 4;
                    goto case 513;
                case 1026: //dxt3
                    ...
                    return Image.LoadPixelData<Bgra32>(destinationData, width, height).CloneAs<Rgba32>();
                case 2050:
