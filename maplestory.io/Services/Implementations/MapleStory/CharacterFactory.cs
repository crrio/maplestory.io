...
namespace maplestory.io.Services.Implementations.MapleStory
{
    public class CharacterFactory : NeedWZ, ICharacterFactory
    {
        ...
        public CharacterFactory(IItemFactory itemFactory, IZMapFactory zMapFactory, ILogger<CharacterFactory> logger)
        {
            _logger = logger;
        ...
        public string[] GetActions(params int[] itemEntries)
        {
            ...
            IEnumerable<WZProperty> itemNodes = WZ.Resolve("Character").Children
                .Where(c => c.Type != PropertyType.Image)
                .SelectMany(c => c.Children)
                .Where(c => itemEntriesStr.Contains(c.NameWithoutExtension));
            ...
            return itemNodes.Skip(1)
                .SelectMany(c => c.Children.Select(b => b.NameWithoutExtension).Where(firstItemAnimations.Contains))
                .Distinct()
                .ToArray();
        }
