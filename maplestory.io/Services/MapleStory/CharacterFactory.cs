...
namespace maplestory.io.Services.MapleStory
{
    public class CharacterFactory : ICharacterFactory
    {
        ...
        public Bitmap GetCharacter(int id, string animation = "stand1", int frame = 0, bool showEars = false, int padding = 2, params int[] items)
        {
            ...
            Dictionary<string, BodyPart> frameImages = bodyAnimation.Frames[frame].Parts;

            Dictionary<string, Point> positions = new Dictionary<string, Point>()
            {
                { "navel", new Point(0, 0) }
            };
            List<Tuple<Point, IFrame>> elements = new List<Tuple<Point, IFrame>>();
            List<IFrame> parts = frameImages.Values.Where(c => showEars || c.Name != "ear").Select(c => (IFrame)c).ToList();

            //while (parts.Count > 0)
            //{
            //    IFrame part = parts.Where(c => c.MapOffset.Any(b => positions.ContainsKey(b.Key))).FirstOrDefault();
            //    if (part == null)
            ...
            //    elements.Add(new Tuple<Point, IFrame>(withOffset, part));
            //    parts.Remove(part);
            //}



            int minX = elements.Select(c => c.Item1.X).Min();
            int maxX = elements.Select(c => c.Item1.X + c.Item2.Image.Width).Max();
            int minY = elements.Select(c => c.Item1.Y).Min();
            int maxY = elements.Select(c => c.Item1.Y + c.Item2.Image.Height).Max();
            ...
            {
                foreach (Tuple<Point, IFrame> element in elements)
